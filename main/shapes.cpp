...
)
{
   ...
   float halfT = 0.5f;   // base thickness in local Z
...
// --- Side: base edge (p0-p2) (your old code was fine here) ---
Vec3f nBase = normalize(Vec3f{ 0.f, -1.f, 0.f });  // pointing down
localPositions.push_back(p0b); localNormals.push_back(nBase);
localPositions.push_back(p2b); localNormals.push_back(nBase);
localPositions.push_back(p2f); localNormals.push_back(nBase);
...
// *** CHANGE: swap p2f/p2b order so the winding matches nVert ***
Vec3f nVert = normalize(Vec3f{ -1.f, 0.f, 0.f }); // pointing -X
localPositions.push_back(p0b); localNormals.push_back(nVert);
localPositions.push_back(p1f); localNormals.push_back(nVert);
localPositions.push_back(p1b); localNormals.push_back(nVert);
...
// edge = p1 - p2 (direction along hypotenuse)
Vec3f edge = normalize(p1f - p2f);
Vec3f nHyp = normalize(Vec3f{ edge.y, -edge.x, 0.f }); // 2D perp

// *** CHANGE: use (p2b,p1b,p1f) and (p2b,p1f,p2f) to match nHyp ***
